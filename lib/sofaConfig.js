...
SofaConfig.setConfig = (key, value, configPath) => {
  const confPath = path.join(configPath, 'Sofa.config.js');
  if (!fs.existsSync(confPath)) {
    createFolder(confPath);
    fs.createWriteStream(confPath);
    // 从当前包中寻找默认配置文件
    const file = fs.readFileSync(path.join(__dirname, '../conf/default.config.js'), 'utf-8');
    try {
      fs.writeFileSync(confPath, file);
      console.log(chalk.green('创建 Sofa.config.js 成功'));
    } catch (error) {
      console.log(chalk.red('创建 Sofa.config.js 失败'));
    }
  }
  const options = {
    text: addConfig(key, value, confPath),
    filePath: confPath,
  };

  try {
    const formatted = format(options);
    fs.writeFileSync(confPath, formatted);
