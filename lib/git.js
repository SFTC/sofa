...
//   subject: 'git --no-pager log --pretty=format:"%s" -n1',
  author: 'git log --all --format=\'%aN <%cE>\' | sort -u',
...
var gitInfo = function(gitDataToGet, cb) {
  ...
  const p = function(resolve, reject) {
    async.each(filtered, 
      execGitCommand.bind(null, responseObject), 
      function(err) {
        if (err) {
          reject(err);
        } else {
          // And run callback with results when finished
          resolve(cb(err, responseObject));
        }
      }
    )
  };
...
gitInfo(['author','user'], (err, result) => {
